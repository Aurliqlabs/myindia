...
export interface SchoolAuditCase {
  ...
  legalReviewedOn?:string; status:EvidenceStatus; publishedOn?:string; fictionalComposite:true;
}
...
export function recordSchoolResponse(world:WorldState,id:string,response:"acknowledged"|"denied"|"no_response",summary:string):void {
  ...
  if(response==="no_response"&&world.date<=entry.requestedResponseOn)throw new Error("Allow time for an official response");
